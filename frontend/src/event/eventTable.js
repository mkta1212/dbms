...
function Row (props) {
    ...
    if (event.periodList.includes(",")){
        startPeriod = event.periodList.split(",")[0]
        endPeriod = parseInt(event.periodList.split(",")[event.periodList.split(",").length-1])+1
    }
    else{
        startPeriod = event.periodList
        endPeriod = parseInt(event.periodList)+1
    }
    return (
      <>
